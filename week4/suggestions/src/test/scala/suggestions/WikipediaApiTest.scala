...
class WikipediaApiTest extends FunSuite {
  ...
  test("WikipediaApi should correctly recover a stream") {
    // TODO: How to test if stream errors?
    val stream = Observable(1, 2, 3)
    var count = 0
    var completed = false

    val sub = stream.recovered.subscribe(
      x => { assert(x == Success(x.get)); count += 1},
      t => assert(false, s"stream error $t"),
      () => completed = true
    )
