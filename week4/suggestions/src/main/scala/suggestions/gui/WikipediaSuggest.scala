...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = searchTerms.concatRecovered { term =>
      wikiSuggestResponseStream(term).timedOut(1)
    }
    ...
    val pages: Observable[Try[String]] = selections.concatRecovered { sel =>
      wikiPageResponseStream(sel).timedOut(1)
    }

    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(response) => { editorpane.text = response }
        case Failure(e) => { status.text = e.getMessage }
      }
    }
