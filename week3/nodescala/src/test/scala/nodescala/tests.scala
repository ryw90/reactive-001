...
class NodeScalaSuite extends FunSuite {
  ...
  test("any works on success") {
    val fs = List(Future{ Thread.sleep(200); 1}, Future{ Thread.sleep(100); 2})
    val any = Future.any(fs)
    assert(Await.result(any, 1 second) == 2)
  }

  test("any works with failure") {
    val fs = List(Future{ Thread.sleep(200); 1}, Future{ Thread.sleep(100); throw new Exception })
    val any = Future.any(fs)
    try {
  ...
  test("delay") {
    val f1 = Future.delay(100 millis)
    val res = Await.result(f1, 200 millis)
    assert(res == ())

    val f2 = Future.delay(200 millis)
    try {
  ...
  test("now") {
    ...
    val f2 = Future { Thread.sleep(100); 1}
    try {
      f2.now
      assert(false)
    } catch {
